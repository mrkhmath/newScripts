...
# Load the Excel file
file_path = 'C:/Users/NCE/Downloads/MarkEnteredReport 23T1.csv'  # Update this with the path to your Excel file
data = pd.read_csv(file_path)

# Define a function to calculate the statistics
def calculate_score_statistics(df, column_name="T1 Marks"):
...
grade_subject_stats[['60 orAbove', 'Above 75%', 'Below 60%']] = pd.DataFrame(grade_subject_stats['Stats'].tolist(), index=grade_subject_stats.index)
...
average_stats = grade_subject_stats.groupby('Subject Name')[['60 orAbove', 'Above 75%', 'Below 60%']].mean().reset_index()
...
plt.bar(ind, average_stats['60 orAbove'], width, label='60 orAbove', color='green')
